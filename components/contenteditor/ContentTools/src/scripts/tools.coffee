...
class ContentTools.Tools.Bold extends ContentTools.Tool
    ...
    @apply: (element, selection, callback) ->
        ...
        [from, to] = selection.get()
        if @className
            tag = new HTMLString.Tag(@tagName, {class: @className})
        else
            tag = new HTMLString.Tag(@tagName)
...
class ContentTools.Tools.Styled extends ContentTools.Tools.Bold

    # Make the current selection of text (non)italic (e.g <i>foo</i>).

    ContentTools.ToolShelf.stow(@, 'styled')
